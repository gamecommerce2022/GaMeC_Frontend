...
const TabsDiscountItem = () => {
 return (<div style={{ backgroundImage: `url('https://source.unsplash.com/random/1920x1080/?black')` }} className='rounded-md'>
   ...
   <TabsBody>
    {tabsData.map(({ key, value }) => (
     <TabPanel key={key} value={key} className='w-full'>
      <DiscountItem key={value.id} id={value.id} image={value.image} title={value.title} productName={value.productName} productPrice={value.productPrice} descriptions={value.descriptions} buttonTitle={value.buttonTitle} link={value.link} />
     </TabPanel>
    ))}
   </TabsBody>
  </Tabs>
