...
export const UploadCard = (props: { onChange: (file
    : FileList | null) => void; multiple?:boolean}) => {
    return (<label htmlFor="file-upload" className='w-[80%] h-[80%] space-y-1 text-center cursor-pointer'>
        <input id="file-upload" name="file-upload" type="file" className="sr-only" accept="image/png, image/jpeg" onChange={(event) => {props.onChange(event.target.files)}} multiple/>
        <svg
