...
export const UploadCardBig = (props: { onChange: (files: FileList | null) => void}) => {
    return (<label htmlFor="file-upload" className='w-[80%] h-[80%] space-y-1 text-center cursor-pointer'>
        <input id="file-upload" name="file-upload" type="file" className="sr-only" accept="image/png, image/jpeg" onChange={(event) => props.onChange(event?.target.files)}/>
        <svg
...
export const ImageCardBig = (props: { image: string}) => {
    return (<div className='w-[80%] h-[80%] flex items-center justify-center mx-auto relative'>
        <img src={props.image} alt={props.image} className="w-[80%] h-[80%] object-fill rounded-xl hover:opacity-90" />
    </div>)
}
    ...
    return (<form><div className={`${props.style}`}>
        ...
        <div className="mt-1 flex justify-center rounded-md border-2 border-dashed border-gray-300 px-6 pt-5 pb-6">
            ...
            <div className="block" onClick={() => props.onImage!(null) }>
                        {props.image !== null ? <ImageCardBig image={props.image}  /> : <UploadCardBig onChange={(files) => {
                            if (files && files[0]) {
