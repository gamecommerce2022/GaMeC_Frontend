...
const filters = [
  ...
  { id: 3, name: 'Address', value: 'address' },
];
...
export const UserTableComponent = () => {
  ...
  function searchText(
    ...
    setLoading(true);
    UserService.get(page || 0, perPage || 30, filter, isDES ? 'DES' : 'ASC', query).then(
      (response) => {
  ...
  function getMaxPage(perPage?: number, query?: string) {
    UserService.getTotalPage(perPage || 30, query).then((response) => {
  ...
  return (
    <div className="">
      ...
      <TableComponent
        ...
        headerList={['ID', 'NAME', 'EMAIL', 'ADDRESS', '']}
        bodyList={userList.map((user, index) => {
          return <UserItemComponent user={user} index={index + 1} key={`${index}-${user._id}`} />;
        })}
      />
