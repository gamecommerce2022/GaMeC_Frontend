...
export const ProductImage: React.FC<{ images: string[] }> = ({ images }) => {
  ...
  const Card: React.FC<{ image: string }> = ({ image }) => {
    return (
      <div className="mt-6 bg-black aspect-w-3 aspect-h-4" onClick={() => setImgMain(image)}>
        <img className="object-fill w-full h-full rounded-sm bg-white" src={image} alt={image} />
      </div>
    );
  };
