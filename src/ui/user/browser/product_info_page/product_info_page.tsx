  ...
  useEffect(() => {
    const getProduct = async () => {
      const productInfo = await axios.get(
        `https://api.rawg.io/api/games/${params.id}?key=79696f0d5de243dd8f873c69c0ea6fce`
      );
  ...
  return (
    <div className="mx-auto max-w-screen-2xl py-12 px-12 bg-black">
      ...
      <div className="flex flex-row py-4 px-12 bg-white w-full">
        ...
        <img
          className="h-[28rem] w-2/3 rounded-lg overflow-visible"
          src={product?.background_image_additional}
          alt={product?.name}
        />
        <div className="flex flex-col ml-12 w-1/3">
          <img
            className="h-48 w-full rounded-lg overflow-visible"
      ...
      <div className="mt-8 text-sm font-medium text-white">
        Product Short Introduction:
        {product?.description}
      </div>

      <div className="flex flex-row px-12 mt-8">
        <div className="w-1/2 h-40 bg-light-green-200 mr-4">
          <div className="text-base flex flex-row flex-wrap font-medium text-white">
            Product Type:
