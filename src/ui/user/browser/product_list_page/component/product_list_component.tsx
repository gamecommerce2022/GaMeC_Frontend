...
export const ProductListComponent: React.FC = () => {
 ...
 const [products, setProducts]: [ProductCardEntity[], (products: ProductCardEntity[]) => void] = useState(defaultProduct);
 ...
 useEffect(() => {
  axios
   .get<ProductPageList>("https://api.rawg.io/api/games?key=79696f0d5de243dd8f873c69c0ea6fce&page_size=32", {
    headers: {
 ...
 return (
  <div className="flex flex-col justify-center space-y-8 w-3/4 cursor-default select-none">
   ...
   <div className='grid grid-rows-4 grid-cols-4 gap-x-8 gap-y-7'>
    {products.map((item) => <ProductCardComponent url={item.url} name={item.name} publisher={item.publisher} platform={item.platform} price={item.price} id={item.id} />)}
   ...
   <div className="grow-0 flex flex-row justify-center h-7 text-white space-x-4">
    <Button variant="outlined" color='blue-gray' className="flex items-center justify-center px-2 py-4" onClick={loadPrev}> <ArrowSmallLeftIcon className="w-8 h-8" /></Button>
    ...
    <Button variant="outlined" color='blue-gray' className="flex items-center justify-center px-2 py-4" onClick={loadNext}> <ArrowSmallRightIcon className="w-8 h-8" /></Button>
   </div>
